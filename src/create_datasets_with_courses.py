...
def build_dictionary(data, course_to_label):
    """
    Returns a list of (label, review, course) tuples
    """
    train_pairs_course = []
    test_pairs_course = []
    train_pairs = []
    ...
    for course in course_to_label.keys():
        ...
        # Add each review to dictionary with correct label and course name
        ...
        for review in course_reviews:
            ...
            n = random.random()
            if n < 0.8:
                train_pairs_course.append((label, cleaned_review, course))
                train_pairs.append((label, cleaned_review))
            else:
                test_pairs_course.append((label, cleaned_review, course))
                test_pairs.append((label, cleaned_review))

    return train_pairs, test_pairs, train_pairs_course, test_pairs_course
...
def main():
    ...
    train_pairs, test_pairs, train_pairs_course, test_pairs_course = build_dictionary(data, course_to_label)

    create_tsv_file(train_pairs, "../data/train_data.tsv")
    create_tsv_file(test_pairs, "../data/test_data.tsv")
    create_tsv_file(train_pairs_course, "../data/train_data_course.tsv")
    create_tsv_file(test_pairs_course, "../data/test_data_course.tsv")
